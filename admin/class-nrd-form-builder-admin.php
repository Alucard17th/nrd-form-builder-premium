...
class Nrd_Form_Builder_Admin {
	...
	public function nrd_plugin_setup_updater() {
		error_log('NRD Updater: Starting nrd_plugin_setup_updater');

		// Only check updates if license is active
		if ( get_option('nrd_form_bd_license_active') !== 'active' ) {
			error_log('NRD Updater: License inactive, skipping update check');
			return;
		}
		...
		$repo_url     = 'https://github.com/Alucard17th/nrd-form-builder---Premium-V1';
		...
		try {
			...
			error_log('NRD Updater: Setup completed successfully');

		} catch (Exception $e) {
			error_log('NRD Updater Exception: ' . $e->getMessage());
		}
	}
	...
	public function custom_dashboard_page_html() {
		...
		if (!empty($_GET['notice']) && !empty($_GET['notice_class'])) {
			echo '<div class="' . esc_attr($_GET['notice_class']) . '"><p>' . esc_html($_GET['notice']) . '</p></div>';
		}
		?>
	...
	public function custom_meta_box_html_drive_sheet($post) {
		...
		if ($screen->base == 'post' && isset($_GET['action']) && $_GET['action'] == 'edit') {
			...
			if (is_array($j) && !empty($j['client_email'])) $sa_email = $j['client_email'];
			}
	...
	public function save_nrd_license_response() {
		...
		update_option( 'nrd_form_bd_license_active', $status );
		if($status == 'active') {
		...
		wp_send_json_success( 'License Updated to ' . $status );
	}
	...
	public function check_license_status() {
		...
		// Make a request to your license verification endpoint
		$response = wp_remote_post(NRD_API_BASE_URL . 'activate-license', [
			'body' => [
		...
		error_log(print_r($response, true));  // Logs the entire response
	...
	public function show_license_warning() {
		...
		if (get_option('nrd_form_bd_license_active') !== 'active') {
			echo '<div class="notice notice-error"><p>Your license is inactive. Please activate your license to continue using the premium features.</p></div>';
		}
	}
	...
	public function ajax_test_sheets() {
		...
		$sheetId  = get_option(self::OPT_SHEET_ID, '');
		$sheetTab = get_option(self::OPT_SHEET_TAB, 'Leads');
		if (!$sheetId) wp_send_json_error(['error' => 'Default Spreadsheet ID is empty. Save settings first.'], 400);
		...
		if ($res === true) wp_send_json_success(['ok'=>true,'message'=>'Row appended successfully.']);
		wp_send_json_error(['error'=>$res], 500);
	}
