...
class Nrd_FB_GSA_Token {
  ...
  public function fetch_access_token(): array {
    ...
    if (!$jwt) return ['error' => 'jwt_sign_failed'];
    ...
    return ($code === 200 && !empty($body['access_token'])) ? $body : ['error' => $body['error_description'] ?? 'token_error'];
  }
  private function b64url($data) {
    return rtrim(strtr(base64_encode($data), '+/', '-_'), '=');
  }
  private function jwt($header, $claim) {
    $segments = [$this->b64url(json_encode($header)), $this->b64url(json_encode($claim))];
...
class Nrd_FB_Sheets_Service {
  ...
  private function load_sa(): array|string {
    ...
    if (!$enc) return 'Missing Service Account JSON. Save it in settings.';
    $plain = Nrd_FB_Secrets::decrypt($enc);
    if (!$plain) return 'Failed to decrypt Service Account JSON.';
  ...
  private function get_token(array $sa) {
    ...
    if ($tok && !empty($tok['access_token'])) return $tok;
  ...
  public function append_row(array $leadData, string $sheetId, string $sheetName, bool $ensure_headers = true) {
    $sa = $this->load_sa();
    if (is_string($sa)) return $sa;

    $tok = $this->get_token($sa);
    if (!empty($tok['error'])) return 'Token error: ' . $tok['error'];
    ...
    if ($ensure_headers) {
      ...
      $existing = wp_remote_get("https://sheets.googleapis.com/v4/spreadsheets/$sheetId/values/$range", [
      ...
      if (!$hasHeaders) {
        $payload = ['values' => [ $headers ]];
        $resp = wp_remote_post("https://sheets.googleapis.com/v4/spreadsheets/$sheetId/values/".rawurlencode("$sheetName!A1").":append?valueInputOption=RAW", [
